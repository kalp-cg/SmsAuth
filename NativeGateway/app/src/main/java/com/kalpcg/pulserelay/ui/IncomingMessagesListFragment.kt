...
import com.kalpcg.pulserelay.databinding.FragmentIncomingMessagesListBinding
import com.kalpcg.pulserelay.modules.incoming.vm.IncomingMessagesListViewModel
import com.kalpcg.pulserelay.ui.adapters.IncomingMessagesAdapter
...
class IncomingMessagesListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.searchIncomingInput.doAfterTextChanged {
            viewModel.setSearchQuery(it?.toString().orEmpty())
        }

        binding.incomingTypeFilterGroup.setOnCheckedStateChangeListener { _, checkedIds ->
            val selectedId = checkedIds.firstOrNull() ?: R.id.filterIncomingAll
            val filter = when (selectedId) {
                R.id.filterIncomingSms -> IncomingMessagesListViewModel.TypeFilter.SMS
                R.id.filterIncomingDataSms -> IncomingMessagesListViewModel.TypeFilter.DATA_SMS
                R.id.filterIncomingMms -> IncomingMessagesListViewModel.TypeFilter.MMS
                else -> IncomingMessagesListViewModel.TypeFilter.ALL
            }
